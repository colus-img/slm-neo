...
import IdentifierCollector from "./filters/identifier_collector.js";
...
p.src = function (src, options = {}) {
	options._identifiers = new Set();
	options._locals = new Set();
	const compiledContent = this._engine.exec(src, options);

	let header = "";
	if (
		options.autoDestructuring !== false &&
		options._identifiers &&
		options._identifiers.size > 0
	) {
