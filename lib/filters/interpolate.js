...
import parsePipeline from "../utils/pipeline_parser.js";
...
p.on_slm_interpolate = function (exps) {
	...
	do {
		...
		if (m) {
			...
			code = parsePipeline(escape ? code : code.slice(1), null, this.options);

			block.push(["slm", "output", escape, code, ["multi"]]);
		} else {
