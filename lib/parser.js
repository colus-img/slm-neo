const attrDelimRe = /^\s*([\(\)\[\]])/;
const blockExpressionRe = /^\s*:\s*/;
const closedTagRe = /^\s*\/\s*/;
const delimRe = /^[\(\[]/;
const doctypeRe = /^doctype\b/i;
const embededRe = /^(\w+):\s*$/;
const emptyLineRe = /^\s*$/;
const htmlCommentRe = /^\/!(\s?)/;
const htmlConditionalCommentRe = /^\/\[\s*(.*?)\s*\]\s*$/;
const indentRegex = /^[ \t]+/;
const indentationRe = /^\s+/;
const newLineRe = /\r?\n/;
const nextLineRe = /[,\\]$/;
const outputBlockRe = /^=(=?)([<>]*)/;
const outputCodeRe = /^\s*=(=?)([<>]*)/;
const tabRe = /\t/g;
const textBlockRe = /^((\.)(\s|$))|^((\||')(\s?))/;
const textContentRe = /^( ?)(.*)$/;

const tagRe = /^(?:#|\.|\*(?=[^\s]+)|(\w+(?:\w+|:|-)*\w|\w+))/;
const attrShortcutRe = /^([\.#]+)((?:\w+|-)*)/;
...
const _parseHtmlComment = (parser, m) => {
	parser._pushOnTop([
		"html",
		"comment",
		[
...
const _parseHtmlConditionalComment = (parser, m) => {
	const block = ["multi"];
	parser._pushOnTop(["html", "condcomment", m[1], block]);
	parser._stacks.push(block);
};

const _parseTextBlock = (parser, m) => {
	let char, space;
	if (m[2] === undefined) {
		char = m[5];
		space = m[6];
...
const _parseOutputBlock = (parser, m) => {
...
p._parseBrokenLine = function () {
	let brokenLine = this._line.trim(),
		m;
...
p._syntaxError = function (message) {
	let column =
		this._origLine !== null && this._line !== null
			? this._origLine.length - this._line.length
			: 0;
	column += 1;
	const msg = [
		message,
