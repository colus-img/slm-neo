...
describe("Interpolate", () => {
	...
	test("interpolation in attribute", () => {
		...
		).toEqual('<p id="asomeidb">hello world</p>');
	});
	...
	test("nested interpolation in attribute", () => {
		...
		).toEqual('<p id="abc${1+1}(">Hello World from @env</p>');
	});
	...
	test("Text interpolation: Expected closing }", () => {
		...
		expect(() => {
			template.render(src, {});
		}).toThrow("Text interpolation: Expected closing }");
	});
	...
	test("interpolation in tag", () => {
		...
		expect(template.render(src, { helloWorld: "Hello" })).toEqual(
			"<p>Hello</p>",
		);
	});
