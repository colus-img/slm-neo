	...
	test(".exec()", () => {
		...
		expect(parser.exec("p nice")).toEqual([
			...
			[
				...
				"p",
				["html", "attrs"],
				["slm", "text", ["multi", ["slm", "interpolate", "nice"]]],
			],
			["newline"],
		]);
		...
		expect(parser.exec("p.alert")).toEqual([
			...
			[
				...
				"tag",
				"p",
				["html", "attrs", ["html", "attr", "class", ["static", "alert"]]],
				["multi", ["newline"]],
			],
		]);

		expect(parser.exec('a href="http://anjlab.ru" AnjLab')).toEqual([
			"multi",
			[
				"html",
				"tag",
				...
				[
					...
					[
						...
						"attr",
						"href",
						["escape", true, ["slm", "interpolate", "http://anjlab.ru"]],
					],
				],
				["slm", "text", ["multi", ["slm", "interpolate", "AnjLab"]]],
			],
			["newline"],
		]);

		expect(parser.exec("/[if IE]\n    p Get a better browser.")).toEqual([
			"multi",
			[
				"html",
				"condcomment",
