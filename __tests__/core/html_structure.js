...
describe("Html structure", () => {
	...
	test("test context", () => {
		...
		expect(result).toEqual(
			'<html><head><meta content="World" name="keywords" /></head><body><p>Hello, World</p><p>Partial Layout</p><strong>The partial is the best</strong><p>nice</p><strong>super!!! World</strong></body></html>',
		);
	});
	...
	test("test current context in partials by default", () => {
		...
		vm.cache(
			compileOptions.filename,
			template.exec(["p Partial ${this.who}"].join("\n"), compileOptions, vm),
		);
		...
		expect(result).toEqual(
			"<p>Current World</p><p>Partial World</p><p>Current Another</p><p>Partial Another</p>",
		);
	});
});
