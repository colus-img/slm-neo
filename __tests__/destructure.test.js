...
describe('Destructuring (this omission)', () => {
  it('should automatically extract variables safely', () => {
    const src = 'p = title\np = user.name\np = Math.round(price)';
    const result = slm.render(src, { title: 'Hello', user: { name: 'Slm' }, price: 100.5 });
    expect(result).toBe('<p>Hello</p><p>Slm</p><p>101</p>');
  });

  it('should fallback to filters if missing in data', () => {
    const src = 'p = upper(title)';
    const result = slm.render(src, { title: 'hello', filters: { upper: (v) => v.toUpperCase() } });
  ...
  it('should allow disabling autoDestructuring', () => {
    ...
    expect(() => slm.render('p = title', { title: 'hello' }, { autoDestructuring: false })).toThrow();
    const result = slm.render(src, { title: 'Explicit' }, { autoDestructuring: false });
  ...
  it('should respect destructuringExclude option', () => {
    ...
    expect(() => slm.render('p = myVar', { myVar: 123 }, { destructuringExclude: ['myVar'] })).toThrow();
  });

  it('should not break with var re-declarations in loops', () => {
    const src = '- for (var i = 1; i <= 3; i++)\n  p = i\n- for (let j = 1; j <= 2; j++)\n  p = j';
